...
const WalletBanner = () => {
  const { ready, initError, initStage, chainId, syncUnlocked, syncHeight } = useContext(LineraContext);
    ...
    <div className="wallet_banner">
      {!ready ? (
        ...
        <>
          ...
          {syncHeight != null && <div className="wallet_banner_line">Height: {syncHeight}</div>}
        </>
      )}
    </div>
  );
